...
export default function ASCIIAnimation({
  ...
  useEffect(() => {
    ...
    const loadPreview = async () => {
      ...
      if (!source) {
        console.error(`ASCIIAnimation: could not find frames for "${frameFolder}"`)
        setIsLoading(false)
        return
      }
  ...
  useLayoutEffect(() => {
    ...
    const updateScale = () => {
      ...
      const newScale = Math.min(availableWidth / naturalWidth, availableHeight / naturalHeight)
  ...
  if (isLoading && frames.length === 0) {
    return (
      <div className={`flex items-center justify-center w-full h-full ${className}`}>
        <div className="w-5 h-5 border border-[var(--border)] border-t-[var(--text-dim)] rounded-full animate-spin" />
      </div>
    )
  }
  ...
  return (
    ...
    >
      {showFrameCounter && (
        <div
          ref={frameCounterRef}
          className="absolute top-2 left-2 z-10 bg-[var(--bg-surface)]/50 px-2 py-1 rounded text-xs font-[family-name:var(--font-geist-mono)]"
        >
